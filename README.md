...
With K-means clustering, data points are classified into \textit{`K'} groups, where `K' represents the number of clusters based on the distance from each group’s centroid. The data points closest to a given centroid will be clustered under the same category. A larger \textit{`K'} value will indicate smaller groupings with more granularity whereas a smaller \textit{`K'} value will have larger groupings and less granularity. As part of the steps in the K-means clustering, instead of a random selection of a number of clusters, we calculate an optimal number based on the scientific data using the elbow method. Using a python script, the processed data obtained from CubeSat was clustered using the K-means algorithm. With the domain expert analysis, the cluster(s) representing ``bad states" and ``good states" are identified. The diagram~\ref{fig6} is the graph showing the number of clustering where the point of inflection on the curve is the best value for \textit{K} while figure~\ref{fig7} is the graph of the K-means data analysis of the scientific data. 
